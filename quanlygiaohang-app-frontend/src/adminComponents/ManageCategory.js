...
const ManagerCategory = () => {
    ...
    const loadCategory = async (page) => {
        try {
            let url = `${endpoint['get-all-cate']}?page=${page - 1}`
            const res = await authApi().get(url)
        
            setTotalPages(res.data.page.totalPages)
            setCategory(res.data._embedded.categoryList)
            setLoading(false)
        }
        ...
        <>
            <Form>
                <Form.Group className="mb-3 p-3" style={{ border: "solid 1px red", width: "40%" }}>
                    <Form.Label>Thêm loại hàng</Form.Label>
